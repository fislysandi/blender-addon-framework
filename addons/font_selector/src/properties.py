...
from .core import load_fonts as lf
from .preferences.addon_preferences import get_addon_preferences
from .operators import switch_font_operator as sw
...
def clear_font_datas():
    for font in bpy.data.fonts:
        if font.name != "Bfont Regular" and font.users == 0:
...
def family_type_update(self, context):
    ...
    if target_font is None:
        if debug:
            print(
                f"FONTSELECTOR --- Update cancelled, unable to get font file : {target_font_props.name}"
            )
        return
...
def family_selection_update(self, context):
    ...
    if font_props.no_callback or self.family_index == -1:
        if debug:
            print("FONTSELECTOR --- Update function cancelled")
        return
...
def register():
    ...
    bpy.types.WindowManager.fontselector_properties = bpy.props.PointerProperty(
        type=FONTSELECTOR_PR_properties,
        name="Font Selector Properties",
    )
    bpy.types.TextCurve.fontselector_object_properties = bpy.props.PointerProperty(
        type=FONTSELECTOR_PR_object_properties,
        name="Font Selector Properties",
    )
    bpy.types.TextStrip.fontselector_object_properties = bpy.props.PointerProperty(
        type=FONTSELECTOR_PR_object_properties,
        name="Font Selector Properties",
    )
...
def unregister():
    if hasattr(bpy.types.WindowManager, "fontselector_properties"):
        del bpy.types.WindowManager.fontselector_properties
    if hasattr(bpy.types.TextCurve, "fontselector_object_properties"):
        del bpy.types.TextCurve.fontselector_object_properties
    if hasattr(bpy.types.TextStrip, "fontselector_object_properties"):
        del bpy.types.TextStrip.fontselector_object_properties

    for cls in (
        FONTSELECTOR_PR_object_properties,
        FONTSELECTOR_PR_properties,
        FONTSELECTOR_PR_font_family_properties,
        FONTSELECTOR_PR_single_font_properties,
    ):
