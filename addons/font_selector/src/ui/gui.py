...
def draw_font_selector(self, context):
    ...
    if (
        ...
        sub.alert = True
        missing_font = (
            f"{active_datas.relink_family_name} - {active_datas.relink_type_name}"
        )
        sub.label(
    ...
    # Font infos
    if active_datas.family_index >= 0 and active_datas.family_index < len(
        props.font_families
    ):
...
def viewport_popover_draw(self, context):
    if get_addon_preferences().viewport_popover and poll_viewport(context):
        self.layout.separator()
        self.layout.popover(
            panel="FONTSELECTOR_PT_viewport_popover", text="", icon="FILE_FONT"
        )
...
def sequencer_popover_draw(self, context):
    if get_addon_preferences().sequencer_popover and poll_sequencer(context):
        self.layout.separator()
        self.layout.popover(
            panel="FONTSELECTOR_PT_sequencer_popover", text="", icon="FILE_FONT"
        )
...
class FONTSELECTOR_OT_popup(bpy.types.Operator):
    ...
    def poll(cls, context):
        if not get_addon_preferences().popup_operator:
            return False
        if (
            context.area.type == "SEQUENCE_EDITOR" and poll_sequencer(context)
...
def unregister():
    for handler, draw_fn in (
        (bpy.types.VIEW3D_HT_header, viewport_popover_draw),
        (bpy.types.SEQUENCER_HT_header, sequencer_popover_draw),
    ):
        try:
            handler.remove(draw_fn)
        except ValueError:
            pass

    for cls in (
        FONTSELECTOR_OT_popup,
        FONTSELECTOR_PT_sequencer_panel,
        FONTSELECTOR_PT_sequencer_popover,
        FONTSELECTOR_PT_properties_panel,
        FONTSELECTOR_PT_viewport_popover,
    ):
