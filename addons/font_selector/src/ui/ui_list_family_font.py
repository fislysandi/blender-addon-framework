...
class FONTSELECTOR_family_uilist(bpy.types.UIList):
    ...
    def draw_item(
        self, context, layout, data, item, icon, active_data, active_propname, flt_flag
    ):
                    ...
                    if flt_flags[idx] != 0:
                        ...
                        if not chk_visible:
                            for f in family.fonts:
                                if (
                                    obj_props.search_font_names
                                    and search in f.font_name.lower()
                                ):
                                    chk_visible = True

                                if (
                                    obj_props.search_filepath
                                    and search in f.filepath.lower()
                                ):
                                    chk_visible = True
...
def unregister():
    for cls in (
        FONTSELECTOR_UL_family_uilist_strip,
        FONTSELECTOR_UL_family_uilist_object,
    ):
