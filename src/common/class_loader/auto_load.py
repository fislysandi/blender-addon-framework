...
def iter_submodule_names(path, root=""):
    for _, module_name, is_package in pkgutil.iter_modules([str(path)]):
        if should_skip_submodule(module_name, root, is_package):
            continue
        if is_package:
...
def should_skip_submodule(module_name: str, root: str, is_package: bool) -> bool:
    """Skip known non-addon runtime modules from auto-load discovery."""
    if module_name.startswith("test_"):
        return True

    if module_name in {"tests", "testing"}:
        return True

    if root.startswith("tests.") or root.startswith("testing."):
        return True
    ...
    if deps_dict:
        ...
        for _ in range(len(deps_dict)):
            ...
            if not resolved:
                # 避免循环依赖
                raise ValueError(
                    f"Circular dependency detected: {list(test_deps.keys())}"
                )
            for k in resolved:
                del test_deps[k]
