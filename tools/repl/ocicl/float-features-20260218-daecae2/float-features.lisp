...
(declaim (inline short-float-fields
                 fields-short-float
                 single-float-fields
                 fields-single-float
                 double-float-fields
...
(deftype sign () '(or (integer -1 -1) (integer +1 +1)))

(defmacro define-float-decoder (type mantissa exponent)
  (let ((name (intern (format NIL "~a-~a" (string type) (string :fields)))))
    `(progn
       (declaim (ftype (function (,type) (values (unsigned-byte ,mantissa) (signed-byte ,exponent) sign &optional)) ,name))
       (defun ,name (,type)
         (declare (optimize (safety 0) (debug 1) speed))
         (let ((int (,(intern (format NIL "~a-~a" (string type) (string :bits))) ,type)))
           (values (ldb (byte ,mantissa 0) int)
                   (- (ldb (byte ,exponent ,mantissa) int) (1- (ash 1 ,(1- exponent))))
                   (if (= 0 (ldb (byte 1 ,(+ exponent mantissa)) int)) +1 -1)))))))

(defmacro define-float-encoder (type mantissa exponent)
  (let ((name (intern (format NIL "~a-~a" (string :fields) (string type)))))
    `(progn
       (declaim (ftype (function ((unsigned-byte ,mantissa) (signed-byte ,exponent) sign) (values ,type &optional)) ,name))
       (defun ,name (mantissa exponent sign)
         (declare (optimize (safety 0) (debug 1) speed))
         (let ((int 0))
           (declare (type (unsigned-byte ,(+ mantissa exponent 1))))
           (setf (ldb (byte ,mantissa 0) int) mantissa
                 (ldb (byte ,exponent ,mantissa) int) (+ exponent (1- (ash 1 ,(1- exponent))))
                 (ldb (byte 1 ,(+ exponent mantissa)) int) (if (< 0 sign) 0 1))
           (,(intern (format NIL "~a-~a" (string :bits) (string type))) int))))))

(defmacro define-float-field-funs (type max)
  (let* ((exponent (round (log (log (eval max) 2) 2)))
         (mantissa (case exponent
                     (4 10)
                     (7 23)
                     (10 52)
                     (14 112)
    ...
    (if mantissa
        `(progn
           (define-float-decoder ,type ,mantissa ,exponent)
           (define-float-encoder ,type ,mantissa ,exponent))
        `(progn
           (defun ,(intern (format NIL "~a-~a" (string :fields) (string type))) (mantissa exponent sign)
             (declare (ignore mantissa exponent sign))
             (error "Implementation not supported."))
           (defun ,(intern (format NIL "~a-~a" (string type) (string :fields))) (float)
             (declare (ignore float))
             (error "Implementation not supported."))))))

(define-float-field-funs short-float most-positive-short-float)
(define-float-field-funs single-float most-positive-single-float)
(define-float-field-funs double-float most-positive-double-float)
(define-float-field-funs long-float most-positive-long-float)
